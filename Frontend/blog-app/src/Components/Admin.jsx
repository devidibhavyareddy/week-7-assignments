  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await axios.get(
        `https://week-7-assignments-mvfg.onrender.com/admin/users`,
  ...
  const handleToggleBlock = async (userId, isCurrentlyActive) => {
    try {
      if (isCurrentlyActive) {
        await axios.put(
          `https://week-7-assignments-mvfg.onrender.com/admin/user/${userId}`,
          {},
          { withCredentials: true },
        );
      } else {
        await axios.put(
          `https://week-7-assignments-mvfg.onrender.com/admin/user-unblock/${userId}`,
          {},
          { withCredentials: true },
        );
      }
